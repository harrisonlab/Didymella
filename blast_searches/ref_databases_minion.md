...
```bash
for RefGenome in $(ls assembly/external/*/*/genbank/genome.ctg.fa | grep 'ITCC_4638'); do
Organism=$(echo $RefGenome | rev | cut -f4 -d '/' | rev)
Strain=$(echo $RefGenome | rev | cut -f3 -d '/' | rev)
Prefix="${Organism}_${Strain}"
echo $Prefix
NewName=$(echo $RefGenome | sed 's/.ctg.fa/_parsed.ctg.fa/g')
cat $CurDir/$RefGenome | sed "s/gi|.*|dbj|//g" | cut -f1 -d'|' | sed 's/\./_/g' | cut -f1 -d ' ' > $NewName
OutDir=analysis/diagnostics_blast/vs_ref_genomes/$Prefix
mkdir -p $OutDir
CurDir=$PWD
cd $OutDir
rm ${Prefix}_genome.fa
cp -s $CurDir/$NewName ${Prefix}_genome.fa
cd $CurDir
OutDir=analysis/diagnostics_blast/vs_ref_genomes/$Prefix
ProgDir=/home/armita/git_repos/emr_repos/tools/pathogen/blast
# qsub $ProgDir/run_blast2csv.sh analysis/diagnostics_blast/D.pinodella_effP.fa dna $RefGenome $OutDir
qsub $ProgDir/run_blast2csv.sh analysis/diagnostics_blast/D.pinodella_effP.fa dna $NewName $OutDir
done
```
...
```bash
CsvFiles=$(ls analysis/diagnostics_blast/vs_*_genomes/*/*D.pinodella_effP.fa_hits.csv | sort)
Headers=$(echo $CsvFiles | sed 's&analysis/diagnostics_blast/vs_ref_genomes/&&g' | sed 's&analysis/diagnostics_blast/vs_seq_genomes/&&g' | sed 's&analysis/diagnostics_blast/vs_own_genomes/&&g' | sed -r "s&_D.pinodella_effP.fa_hits.csv&&g" | sed "s/\/\w*//g")
OutDir=analysis/diagnostics_blast/vs_ref_genomes/extracted
mkdir -p $OutDir
CsvFiles=$(ls analysis/diagnostics_blast/vs_*_genomes/*/*D.pinodella_effP.fa_hits.csv | sort)
Genomes=$(ls analysis/diagnostics_blast/vs_*_genomes/*/*_genome.fa| sort)
...
```
Note the following gene didnt parse properly in ITCC_4638
```
61B|g9971.t1    9       1       1e-69
```
